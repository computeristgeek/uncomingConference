<?php
	$title='Committees - ICNTSE';
	$keywords='icntse,pua,kth,international conference,new trends,sustainable energy,science,extended abstract,committees';
	$description='This page provides the ICNTSE Conference Committees.';
?>
<?php include 'header.php' ?>
  <body style="padding-top:50px;" id="committeesbody">
<div class="container">
<?php include('navbar.php'); ?>
	    <div class="row">
		<div class="page-header">
		  <h1>Conference Committees</h1>
		</div>
		</div>
	    </div>

<div class="container">
  <!-- Nav tabs -->
  <ul class="nav nav-tabs" role="tablist">
    <li role="presentation" class="active"><a href="#Sience_C" aria-controls="Sience_C" role="tab" data-toggle="tab"><b>Scientific</b></a></li>
    <li role="presentation"><a href="#Stu_C" aria-controls="Stu_C" role="tab" data-toggle="tab"><b>Student</b></a></li>
    <li role="presentation"><a href="#Org_C" aria-controls="Org_C" role="tab" data-toggle="tab"><b>Organizing</b></a></li>
    <li role="presentation"><a href="#PubRel_C" aria-controls="PubRel_C" role="tab" data-toggle="tab"><b>Publicity</b></a></li>
    <li role="presentation"><a href="#Pub_C" aria-controls="Pub_C" role="tab" data-toggle="tab"><b>Publication</b></a></li>
    <li role="presentation"><a href="#Local_C" aria-controls="Local_C" role="tab" data-toggle="tab"><b>Arrangements</b></a></li>
  </ul>

  <!-- Tab panes -->
  <div class="tab-content">
    <div role="tabpanel" class="tab-pane active" id="Sience_C">
    	<h1>Scientific Committee</h1>
    	<table style="display:;" class="table table-hover" id="sciComm">
	    <thead>
		<tr>
		    <th>Name</th>
		    <th>Affiliation</th>
		</tr>
	    </thead>
	    <tbody>
		<tr>
		    <td>Prof. Abdelmonem Mousa</td>
		    <td>(PUA)</td>
		</tr>
		<tr>
		    <td>Prof. Ossama El Masry</td>
		    <td>(PUA)</td>
		</tr>
		<tr>
		    <td>Prof. Mohamed Fahim</td>
		    <td>(PUA)</td>
		</tr>
		<tr>
		    <td>Prof. Adel Abou El Ella *</td>
		    <td>(MU)</td>
		</tr>
		<tr>
		    <td>Prof. Alaa Shebl</td>
		    <td>(PUA)</td>
		</tr>
		<tr>
		    <td>Prof. Ramadan Abdelmaksoud</td>
		    <td>(PUA)</td>
		</tr>
		<tr>
		    <td>Prof. Mohamed Gamal Wasel</td>
		    <td>(PUA)</td>
		</tr>
		<tr>
		    <td>Prof. Mohamed Hasab</td>
		    <td>(AU)</td>
		</tr>
		<tr>
		    <td>Prof. Hassan Farag</td>
		    <td>(AU)</td>
		</tr>
		<tr>
		    <td>Prof. Nabil Abbaasy</td>
		    <td>(AU)</td>
		</tr>
		<tr>
		    <td>Prof. Ahmed Abdallah</td>
		    <td>(MU)</td>
		</tr>
		<tr>
		    <td>Prof. Ahmed Hegazy</td>
		    <td>(MU)</td>
		</tr>
		<tr>
		    <td>Prof. Sabri Abdel-Latif</td>
		    <td>(MU)</td>
		</tr>
		<tr>
		    <td>Prof. Ragab AbdelAziz El-Sehiemy</td>
		    <td>(KFS)</td>
		</tr>
		<tr>
		    <td>Prof. Ahmed Refaat</td>
		    <td>(TU)</td>
		</tr>
		<tr>
		    <td>Prof. Mohamed Orabi</td>
		    <td>(ASWU)</td>
		</tr>
		<tr>
		    <td>Prof. Moataz AbdelAziz</td>
		    <td>(ASU)</td>
		</tr>
		<tr>
		    <td>Prof. Sobhi Seri</td>
		    <td>(PSU)</td>
		</tr>
		<tr>
		    <td>Prof. Nabil Abdelmoniem</td>
		    <td>(CU)</td>
		</tr>
		<tr>
		    <td>Prof. Omar Hanafy</td>
		    <td>(HU)</td>
		</tr>
		<tr>
		    <td>Prof. Mamoun Muhammed</td>
		    <td>(KTH)</td>
		</tr>
		<tr>
		    <td>Prof. Göran Engdahl *</td>
		    <td>(KTH)</td>
		</tr>
		<tr>
		    <td>Prof. Lennar Soder</td>
		    <td>(KTH)</td>
		</tr>
		<tr>
		    <td>Prof. Thomas Ackermann</td>
		    <td>(KTH)</td>
		</tr>
		<tr>
		    <td>Prof. Mark Howells</td>
		    <td>(KTH)</td>
		</tr>
		<tr>
		    <td>Prof. Semida Silveira</td>
		    <td>(KTH)</td>
		</tr>
		<tr>
		    <td>Prof. Andrew Martin</td>
		    <td>(KTH)</td>
		</tr>
		<tr>
		    <td>Prof. Per Lundqvist</td>
		    <td>(KTH)</td>
		</tr>
		<tr>
		    <td>Prof. Olga Kordas</td>
		    <td>(KTH)</td>
		</tr>
		<tr>
		    <td>Prof. Yohannes Kiros</td>
		    <td>(KTH)</td>
		</tr>
		<tr>
		    <td>Prof. Johan Karlander</td>
		    <td>(KTH)</td>
		</tr>
		<tr>
		    <td>Prof. Alexis Pontvik</td>
		    <td>(KTH)</td>
		</tr>
		<tr>
		    <td colspan="2">* Chairs<br />PUA = Pharos University in ALexandria<br />AU = Alexandria University<br />MU = Menoufia University<br />KFS = Kafrelsheikh University<br />TU = Tanta University<br />ASWU = Aswan University<br />ASU = Ain Shams University<br />PSU = Port Said University<br />CU = Cairo University<br />HU = Helwan University<br />KTH = Royal nstitute of Technology</td>
		</tr>
		<tr>
		    <td colspan="2" class="handcursor" onclick="toggle_visibility('role00')">
				Roles and Responsibilities:
				<div style='display:none;' id='role00'><ol>
				<li>Determination of the scientific topics and range of information of the conference.</li>
				<li>Researching and selecting the local and International scientific committee members</li>
				<li>Researching, selecting and recommending the Keynote speakers and session chairmen</li>
				<li>Researching and selecting the independent referees who are knowledgeable in the pertinent subject areas of the conference program. </li>
				<li>Review abstracts of the received papers and select those research ideas that are consistent with the conference program topics in the Call of papers.</li>
				<li>Sending the extended abstracts of the accepted abstracts to the referees for revision, selection and approval. </li>
				<li>Work with the Publication committee on the Preparation of extended abstracts for publication</li>
				</ol></div>
				</td>
		</tr>
	    </tbody>
	</table>
    </div>
    <div role="tabpanel" class="tab-pane" id="Stu_C">
    	<h1>Student Committee</h1>
    	<table class="table table-hover" id="stuComm">
	    <thead>
		<tr>
		    <th>Function</th>
		    <th>Name</th>
		</tr>
	    </thead>
	    <tbody>
		<tr>
		    <td>Head</td>
		    <td>Prof. Ahmed Abdallah</td>
		</tr>
		<tr>
		    <td></td>
		    <td>Dr. Alaa Khalil</td>
		</tr>
		<tr>
		    <td colspan="2" class="handcursor" onclick="toggle_visibility('role09')">
				Roles and Responsibilities:
				<div style='display:none;' id='role09'><ol>
				<li>TBD.</li>
				<li>TBD.</li>
				<li>TBD.</li>
				<li>TBD.</li>
				<li>TBD.</li>
				<li>TBD.</li>
				</ol></div>
			</td>
		</tr>
	    </tbody>
	</table>
    </div>
    <div role="tabpanel" class="tab-pane" id="Org_C">
    	<h1>Organizing Committee</h1>
    	<table class="table table-hover" id="orgComm">
	    <thead>
		<tr>
		    <th>Function</th>
		    <th>Name</th>
		</tr>
	    </thead>
	    <tbody>
		<tr>
		    <td>Head</td>
		    <td>Prof.Ahmed Abd El Metaal</td>
		</tr>
		<tr>
		    <td rowspan="6">Sponsorship & Exhibit</td>
		    <td>Prof. Kadry Montaser</td>
		</tr>
		<tr>
		    <td>Dr.Khalid Hemdan</td>
		</tr>
		<tr>
		    <td>Dr.Hesham El-Shemy</td>
		</tr>
		<tr>
		    <td>Dr.Yaser El-Kamshoushi</td>
		</tr>
		<tr>
		    <td>Dr.Mona Ouf</td>
		</tr>
		<tr>
		    <td>Eng.Mohamed El-Fakharany</td>
		</tr>
		<tr>
		    <td colspan="2" class="handcursor" onclick="toggle_visibility('role03')">
				Roles and Responsibilities:
				<div style='display:none' id='role03'><ol>
				<li>Arranging a plan for promotion of the conference event throughout the planning process according to directions of the Organizing Committee.</li>
				<li>The committee gathers, maintains and utilizes list of media contacts and potential attendees for targeted outreach.</li>
				<li>The committee is responsible for promoting the conference through placement in various publication calendars and advertisements in Websites and news media.</li>
				<li>Arranging for local publicity of the advance program.</li>
				<li>Contacting various archival suppliers/vendors, consultants and any other firms to solicit on paid advertise for inclusion in the “Program brochure” and “Proceeding manual”.</li>
				<li>Responsible for planning and organizing the trade show for suppliers/vendors and service oriented organizations to introduce stimulating state of the art technological displays and informative presentations.</li>
				</ol></div>
				</td>
		</tr>
		<tr>
		    <td rowspan="3">Finance Committee</td>
		    <td>Dr.Mohamed AbdelWahab</td>
		</tr>
		<tr>
		    <td>Dr.Mohamed AbdelKerim</td>
		</tr>
		<tr>
		    <td>Eng.Ramy Moussa</td>
		</tr>
		<tr>
		    <td colspan="2" class="handcursor" onclick="toggle_visibility('role02')">
				Roles and Responsibilities:
				<div style='display:none' id='role02'><ol>
				<li>Controlling all incoming funds and disbursements.</li>
				<li>Opening a separate banking account.</li>
				<li>Maintaining all financial records and paying all invoices subsequent to obtaining appropriate approval.</li>
				<li>Arranging for collecting and documenting on-site registration fees as well as providing receipts, if requested.</li>
				<li>Arranging to have receipt included in the registration packages.</li>
				<li>Arranging for the safe-keeping of all money collected.</li>
				<li>Providing appropriate input in the budgetary process, if and when required.</li>
				<li>Invoicing attendances, Exhibitors…etc, as required.</li>
				<li>Submitting a monthly financial report at the Committee meetings and to the Secretary General.</li>
				<li>Submitting a full financial report to the Secretary General not later than 45 days after the conclusion of the conference or as when required to do so by the conference chairman.</li>
				</ol></div>
				</td>
		</tr>
	    </tbody>
	</table>
    </div>
    <div role="tabpanel" class="tab-pane" id="PubRel_C">
    	<h1>Publicity and Public Relations Committee</h1>
    	<table class="table table-hover" id="pubComm">
	    <thead>
		<tr>
		    <th>Function</th>
		    <th>Name</th>
		</tr>
	    </thead>
	    <tbody>
		<tr>
		    <td>Head</td>
		    <td>Prof.Ramadan AbdelMaksoud</td>
		</tr>
		<tr>
		    <td rowspan="4">Design and Production of Materials</td>
		    <td>Dr.Ghada Ragheb</td>
		</tr>
		<tr>
		    <td>Dr.Mona Nagieb</td>
		</tr>
		<tr>
		    <td>Eng.Engy Nagieb</td>
		</tr>
		<tr>
		    <td>Eng.Mona Salem</td>
		</tr>
		<tr>
		    <td colspan="2" class="handcursor" onclick="toggle_visibility('role04')">
			Roles and Responsibilities:
			<div style='display:none' id='role04'><ol>
			<li>Designing and production of:
			<ul>
			<li>Conference call of paper</li>
			<li>Conference Logo</li>
			<li>Letterhead and envelopes</li>
			<li>Invitation cards for the Opening ceremony and Gala dinner</li>
			<li>Coffee and lunch break tickets</li>
			<li>Announcements for publicity of the Conference events, and</li>
			<li>Session: Signs, Banners and Posters </li>
			</ul>
			</li>
			<li>Production of all printed matters of the conference’ which includes the followings:
			<ul>
			<li>Advanced Flyer.Itis devised to provide advance notice about the event of the Conference.</li>
			<li><b>Countdowns.</b> It is the most common means of informing the membership at large and other interested parties of the upcoming Conference. It consists of a monthly-newsletter for a period of 10 months prior to the Conference and covers the progress of the different activities pertinent to the event.</li>
			<li><b>Call for Abstracts.</b> It is a means of advertising the upcoming Conference and triggering potential speaker’s interest to submit a paper for presentation at this premier scientific event. Its contents should highlight the subject matters to be covered, expected formats and formal application to be returned to the Program Chairperson.</li>
			<li><b>Program Brochure.</b> It serves the purpose of introducing the upcoming Conference program more to potential attendees. It provides information that will enable them to select program sessions, meetings and other events that are scheduled. In addition, it provides attendees with the required forms to register for the Conference, make necessary travel and housing arrangements.</li>
			<li><b>Proceedings Manual.</b> It could be handled out to attendees who have registered for the full registration package. Other can obtain for a nominal fee a copy of this publication.</li>
			<li><b>Committee Abstract-book & Certificates.</b> It is a comprehensive publication used to recognize the recipients of awards in the Archival profession, the dedicated contribution of all Conference members, the continued support of Exhibitors and finally the printing of presented paper’s Abstracts.</li>
			<li>The Conference Brochures and publications are produced in conjunction with the publication committee.</li>
			<li>The announcements publications for publicity of the Conference eventsare produced in conjunction with the publication committee.</li>
			</ul>
			</li>
			</ol></div>
			</td>
		</tr>
		<tr>
		    <td rowspan="4">Public Relations</td>
		    <td>Dr.Ehssan Nasef</td>
		</tr>
		<tr>
		    <td>Dr.Ahmed AbdelMutal</td>
		</tr>
		<tr>
		    <td>Dr.Hesham El-Shimy</td>
		</tr>
		<tr>
		    <td>Dr.Dalia ElSorady</td>
		</tr>
		<tr>
		    <td colspan="2" class="handcursor" onclick="toggle_visibility('role05')">
			Roles and Responsibilities:
			<div style='display:none' id='role05'> Arranging a plan for promotion of the conference event throughout the planning process<ol>
			<li>The committee gathers, maintains and utilizes list of media contacts and potential attendees for targeted outreach.</li>
			<li>The committee is responsible for promoting the conference through placement in various publication calendars and advertisements in Websites and news media.</li>
			<li>Arranging for local publicity of the advance program.</li>
			<li>Contacting various archival suppliers/vendors, consultants and any other firms to solicit on paid advertise for inclusion in the “Program brochure” and “Proceeding manual”.</li>
			<li>Arranging and supervising the production of various publicity items (i.e. Conference commemorative pins, pens, bags…etc.</li>
			<li>Organizing the reception of invitees and attendees in the Opening ceremony, coffee and lunch breaks, and other conference events.</li>
			</ol></div>
			</td>
		</tr>
	    </tbody>
	</table>
    </div>
    <div role="tabpanel" class="tab-pane" id="Pub_C">
    	<h1>Publications Committee</h1>
    	<table class="table table-hover" id="publComm">
	    <thead>
		<tr>
		    <th>Function</th>
		    <th>Name</th>
		</tr>
	    </thead>
	    <tbody>
		<tr>
		    <td>Head</td>
		    <td>Prof.Kadry Montasser</td>
		</tr>
		<tr>
		    <td rowspan="2">Publication Committee</td>
		    <td>Dr.Noha Shaaban</td>
		</tr>
		<tr>
		    <td>Dr.Ehssan Nasef</td>
		</tr>
		<tr>
		    <td colspan="2" class="handcursor" onclick="toggle_visibility('role06')">
				Roles and Responsibilities:
				<div style='display:none' id='role06'><ol>
				<li>Determination of the scientific topics and range of information of the conference in cooperation with the Scientific committee</li>
				<li>Prepare letters of invitation to potential participants and registration forms. </li>
				<li>Determination of the “Review Process Rules” </li>
				<li>Preparing the “Abstract Format” and the “Extended Abstract Format”.</li>
				<li>Preparing the Conference Copyright Forms for the papers accepted for publications and the proceedings format.</li>
				<li>Select research topics that are consistent with the conference program topicsand form consistent sessions.</li>
				<li>Determination and formulation of the necessary professional ethics rules that referees or anyone with access to conference submissions treat the contents of papers under review as privileged information not to be disclosed to others before publications.    </li>
				</ol></div>
				</td>
		</tr>
		<tr>
		    <td rowspan="5">Website Committee</td>
		    <td>Dr.Amr ElSadany</td>
		</tr>
		<tr>
		    <td>Eng.Karim Soliman</td>
		</tr>
		<tr>
		    <td>Eng.Salahuddin ElKazak</td>
		</tr>
		<tr>
		    <td>Eng.HebaAllah AbdelWahab</td>
		</tr>
		<tr>
		    <td>Eng.Nahla Moustafa</td>
		</tr>
		<tr>
		    <td colspan="2" class="handcursor" onclick="toggle_visibility('role07')">
			Roles and Responsibilities:
			<div style='display:none' id='role07'><ol>
			<li>The committee is responsible for designing and managing the Conference Website</li>
			<li>The committee is responsible for implementing and programming the Conference Website</li>
			<li>The committee is responsible for maintaining the Conference Website</li>
			<li>The committee is responsible for promoting the conference through placement in various publication calendars and advertisements in Websites and news media.</li>
			<li>The committee gathers, maintains and utilizes list of contacts and potential attendees for targeted outreach.</li>
			</ol></div>
			</td>
		</tr>
	    </tbody>
	</table>
    </div>
    <div role="tabpanel" class="tab-pane" id="Local_C">
    	<h1>Local Arrangements Committee</h1>
    	<table class="table table-hover" id="locComm">
	    <thead>
		<tr>
		    <th>Function</th>
		    <th>Name</th>
		</tr>
	    </thead>
	    <tbody>
		<tr>
		    <td>Head</td>
		    <td>Prof.Hassan ElGhazoly</td>
		</tr>
		<tr>
		    <td>Coordinator</td>
		    <td>Dr.Mohamed Abdel Rahman</td>
		</tr>
		<tr>
		    <td rowspan="7">Local Arrangements & Logistics</td>
		    <td>Dr.Mona Othman</td>
		</tr>
		<tr>
		    <td>Dr.Maged Mahmoud</td>
		</tr>
		<tr>
		    <td>Dr.Noha Saeed</td>
		</tr>
		<tr>
		    <td>Dr.Marwa Abdelfattah</td>
		</tr>
		<tr>
		    <td>Dr.Alaa Khalil</td>
		</tr>
		<tr>
		    <td>Eng.Hanan El-Gammal</td>
		</tr>
		<tr>
		    <td>Eng.Marwa Waly</td>
		</tr>
		<tr>
		    <td colspan="2" class="handcursor" onclick="toggle_visibility('role08')">
			Roles and Responsibilities:
			<div style='display:none' id='role08'><ol>
			<li>Planning and organizing the show for presenters and organizations to introduce state of the art technological displays and informative presentations.</li>
			<li>Determining size and location of rooms according to type of session and/or events in consultation with the Coordinator of Program Committee.</li>
			<li>Arranging for audio/visual equipment requirements in all sessions and event rooms in consultation with the Coordinator of Program Committee.</li>
			<li>Arranging for hospitality facilities, including coffee/ lunch breaks and beverages, as required.</li>
			<li>Arranging for coffee/lunch breaks time schedule and liaising with Exhibits coordinator to maximize traffic in exhibit area. </li>
			<li>Arranging, whenever available, for social activities in consultation with the Coordinator of Program Committee.</li>
			<li>Arranging for “Opening Ceremonies” activities in consultation with the Secretary General.</li>
			<li>Arranging and supervising the production of various publicity items (i.e. Conference commemorative pins, pens, bags…etc.</li>
			<li>Organizing the reception of invitees and attendees in the Opening ceremony, coffee and lunch breaks, and other conference events.</li>
			<li>Coordinating and monitoring the set-up for all sessions and events, as instructed.</li>
			<li>Providing an “Inquiry Desk” in the registration area for VIP’s, guests, speakers, attendees, spouses and visitors who are seeking information about the Conference, the City and more.</li>
			</ol></div>
			</td>
		</tr>
	    </tbody>
	</table>
    </div>
  </div>
</div>
<?php include 'footer.php' ?>
